...
Capybara.configure do |config|
  port = 3000
  config.app_host = "http://www.rdv-solidarites.localhost:#{port}"
  # config.asset_host = "http://localhost:#{port}"  # for screenshots
  config.server_host = "www.rdv-solidarites.localhost"
...
def seed
  ...
  territory92 = FactoryBot.create(:territory, departement_number: "92")
  organisation = FactoryBot.create(:organisation, :with_contact, territory: territory92)
  service_medical = FactoryBot.create(:service, name: "Service Médical")
  service_social = FactoryBot.create(:service, name: "Service Social")
  motif_vaccination = FactoryBot.create(:motif, name: "Vaccination", organisation: organisation, restriction_for_rdv: nil, service: service_medical)
  motif_tel = FactoryBot.create(:motif, :by_phone, name: "Télé consultation", organisation: organisation, restriction_for_rdv: nil, service: service_medical)
  motif_collectif = FactoryBot.create(:motif, :collectif, name: "Atelier collectif", organisation: organisation, restriction_for_rdv: nil, service: service_social)
  motif_rsa = FactoryBot.create(:motif, name: "poursuite RSA", organisation: organisation, restriction_for_rdv: nil, service: service_social)
  lieu_centre = FactoryBot.create(:lieu, name: "MDS Centre", organisation: organisation)
  lieu_est = FactoryBot.create(:lieu, name: "MJD Est", organisation: organisation)
  _plage_ouverture_vaccination = FactoryBot.create(:plage_ouverture, :daily, first_day: now + 1.month, motifs: [motif_vaccination], lieu: lieu_centre, organisation: organisation)
  _plage_ouverture_motif_tel = FactoryBot.create(:plage_ouverture, :daily, first_day: now + 1.month, motifs: [motif_tel], lieu: lieu_centre, organisation: organisation)
  _plage_ouverture_rsa = FactoryBot.create(:plage_ouverture, :daily, first_day: now + 1.month, motifs: [motif_rsa], lieu: lieu_est, organisation: organisation)
  agent = FactoryBot.create(:agent, organisations: [organisation])
  3.times do |i|
    FactoryBot.create(
      :rdv,
  ...
  _user = FactoryBot.create(:user, first_name: "Jean", last_name: "Dupont", email: "jean.dupont@lycos.fr", password: "Rdvservicepublictest1!")
end
